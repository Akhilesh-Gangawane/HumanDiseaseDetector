...
﻿-- ============================================================
-- Dhanvantari AI - Complete Database Schema
-- Includes: base schema + doctor verification + admin role
-- ============================================================
-- HOW TO RUN:
--   Step 1: Run everything in this file EXCEPT the last UPDATE
--   Step 2: Run the final UPDATE (admin promotion) separately
--           after Step 1 commits, because PostgreSQL requires
...
CREATE TYPE payment_method AS ENUM ('card', 'upi', 'netbanking', 'cod');
CREATE TYPE risk_level AS ENUM ('High', 'Medium', 'Low');
CREATE TYPE doctor_verification_status AS ENUM ('unverified', 'pending', 'verified', 'rejected');
...
CREATE TABLE doctors (
  ...
  available BOOLEAN DEFAULT TRUE,
  -- Verification fields
  verification_status doctor_verification_status NOT NULL DEFAULT 'unverified',
...
-- Unique registration number per council
CREATE UNIQUE INDEX idx_doctors_reg_number
  ON doctors (medical_council, registration_number)
...
CREATE POLICY "pathology_tests_public_read"   ON pathology_tests   FOR SELECT USING (TRUE);
...
-- Insurance plans
INSERT INTO insurance_plans (name, price_per_year, coverage_amount, is_popular, features) VALUES
  ('Basic Plan',   5000,  300000,  FALSE, ARRAY['Hospitalization coverage','Pre & post hospitalization','Daycare procedures','Ambulance charges','Annual health checkup']),
  ('Premium Plan', 12000, 1000000, TRUE,  ARRAY['All Basic Plan benefits','Maternity coverage','Critical illness cover','No room rent limit','International coverage','Wellness programs']),
  ('Family Plan',  18000, 1500000, FALSE, ARRAY['All Premium Plan benefits','Covers 4 family members','Newborn baby coverage','Restoration benefit','Home healthcare','Mental health coverage']);

-- Knowledge articles
INSERT INTO knowledge_articles (title, category, tag, tag_color, read_mins, stars, description, url, image_url) VALUES
  ('Understanding Type 2 Diabetes', 'Endocrinology', 'Essential', '#0891b2', 5, 4.8, 'Types, symptoms, lifestyle management, and the latest insulin research.',                          'https://www.diabetes.org/diabetes/type-2',                                                          'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=480&q=80'),
  ('Heart Health: A Complete Guide', 'Cardiology',   'Popular',   '#3b82f6', 4, 4.9, 'Prevention, diet, exercise, and early warning signs of cardiovascular disease.',                   'https://www.heart.org/en/health-topics/heart-attack',                                               'https://images.unsplash.com/photo-1530026405186-ed1f139313f8?w=480&q=80'),
  ('Mental Wellness in 2025',        'Psychology',   'New',       '#8b5cf6', 6, 4.7, 'Evidence-based strategies: CBT, mindfulness, sleep hygiene, and digital detox.',                   'https://www.nimh.nih.gov/health/topics/mental-health-medications',                                  'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=480&q=80'),
  ('Nutrition Science Decoded',      'Nutrition',    'Guide',     '#14b8a6', 7, 4.6, 'Macronutrients, micronutrients, fasting, and personalised eating plans.',                          'https://www.healthline.com/nutrition',                                                              'https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=480&q=80'),
  ('Exercise for Every Body',        'Wellness',     'Popular',   '#10b981', 5, 4.8, 'Structured workout plans, recovery science, and injury prevention tips.',                           'https://www.mayoclinic.org/healthy-lifestyle/fitness/basics/fitness-basics/hlv-20049447',           'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=480&q=80'),
  ('Sleep Disorders Explained',      'Sleep Medicine','In-Depth', '#f59e0b', 6, 4.7, 'Insomnia, sleep apnea, circadian rhythm — diagnosis and treatment options.',                       'https://www.sleepfoundation.org/sleep-disorders',                                                   'https://images.unsplash.com/photo-1541781774459-bb2af2f05b55?w=480&q=80');
...
INSERT INTO doctors (id, user_id, specialization, experience_years, available, verification_status, medical_council, registration_number, registration_year, verified_at) VALUES
  ('00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000001', 'General Medicine', 10, TRUE, 'verified', 'Maharashtra Medical Council', 'MMC-000001', 2014, NOW());
...
INSERT INTO predictions (patient_id, patient_name, disease, confidence, symptoms, explanation, method, status) VALUES
  ('00000000-0000-0000-1000-000000000004', 'John Doe',    'Diabetes Type 2', 94, ARRAY['Fatigue','Increased thirst','Frequent urination','Blurred vision'], 'Based on symptoms and patient history, the AI model predicts Type 2 Diabetes with high confidence.', 'Deep Residual Neural Network', 'Pending'),
  ('00000000-0000-0000-1000-000000000001', 'Emma Watson', 'Hypertension',    87, ARRAY['Headache','Dizziness'],                                             'Model suggests moderate risk of Hypertension. Requires blood pressure monitoring.',                  'Deep Residual Neural Network', 'Pending');

-- ============================================================
-- SEED LAB TEST REQUESTS
-- ============================================================
...
-- ADMIN ROLE
-- ============================================================
-- IMPORTANT: Run the two statements below in SEPARATE query
-- tabs because PostgreSQL cannot use a new enum value in the
-- same transaction it was created.
--
-- STEP 1 (run first, wait for success):
--   ALTER TYPE user_role ADD VALUE IF NOT EXISTS 'admin';
--
-- STEP 2 (run in a new tab after Step 1 completes):
...
-- (Separate from the generic orders table — stores medicine
--  e-commerce orders placed via the Buy Medicine page)
-- ============================================================
...
CREATE POLICY "medicine_orders_patient_self" ON medicine_orders
  FOR ALL USING (
...
-- ================= FILE: supabase_migration_appointments.sql =================
...
-- ============================================================
-- Dhanvantari AI — Realtime Fix Migration
-- Run this in your Supabase SQL editor
-- ============================================================
-- This script:
--   1. Creates the 4 missing tables the codebase references
--   2. Enables Supabase Realtime on the key tables
--   3. Adds RLS policies so the anon/service-role keys work
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- 1. doctor_notifications
--    doctor_id → users.id  (the doctor's users row)
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS doctor_notifications (
  id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  doctor_id   UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  patient_id  UUID REFERENCES users(id) ON DELETE SET NULL,  -- optional sender
  title       TEXT NOT NULL,
  message     TEXT NOT NULL,
  type        notification_type NOT NULL DEFAULT 'general',
...
CREATE INDEX IF NOT EXISTS idx_doctor_notifications_doctor_id
  ON doctor_notifications (doctor_id, created_at DESC);

-- ─────────────────────────────────────────────────────────────
-- 2. patient_notifications
--    patient_id → users.id  (the patient's users row)
-- ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS patient_notifications (
  id          UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  patient_id  UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  doctor_id   UUID REFERENCES users(id) ON DELETE SET NULL,   -- optional sender
  title       TEXT NOT NULL,
  message     TEXT NOT NULL,
  type        notification_type NOT NULL DEFAULT 'general',
  read        BOOLEAN NOT NULL DEFAULT FALSE,
...
-- ─────────────────────────────────────────────────────────────
-- 3. doctor_patients  (junction: which patients a doctor has seen)
--    Both columns reference users.id
-- ─────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_medicine_orders_patient_id
  ON medicine_orders (patient_id, created_at DESC);

-- ─────────────────────────────────────────────────────────────
-- 5. Add missing columns to appointments (if not present)
--    These are referenced in the PATCH route but may be absent
-- ─────────────────────────────────────────────────────────────
ALTER TABLE appointments
  ADD COLUMN IF NOT EXISTS doctor_calendar_event_id   TEXT,
  ADD COLUMN IF NOT EXISTS doctor_calendar_event_link TEXT,
  ADD COLUMN IF NOT EXISTS patient_calendar_event_id  TEXT,
  ADD COLUMN IF NOT EXISTS patient_calendar_event_link TEXT,
  ADD COLUMN IF NOT EXISTS initiated_by               TEXT DEFAULT 'patient';

-- ─────────────────────────────────────────────────────────────
-- 6. Enable Supabase Realtime on the tables that need live sync
-- ─────────────────────────────────────────────────────────────
-- Realtime is enabled per-table via the replication publication.
-- Run each ALTER separately if you hit "already member" errors.
...
--    The API routes use the service-role key (bypasses RLS).
--    The client-side realtime subscription uses the anon key,
--    so we need permissive SELECT policies for authenticated users.
-- ─────────────────────────────────────────────────────────────
...
-- doctor_notifications: doctors can read their own
CREATE POLICY "doctors_read_own_notifications"
  ON doctor_notifications FOR SELECT
  USING (true);   -- service-role bypasses; anon reads are filtered by subscription

CREATE POLICY "service_role_all_doctor_notifications"
  ON doctor_notifications FOR ALL
  USING (true);

-- patient_notifications: patients can read their own
CREATE POLICY "patients_read_own_notifications"
  ON patient_notifications FOR SELECT
  USING (true);

CREATE POLICY "service_role_all_patient_notifications"
  ON patient_notifications FOR ALL
  USING (true);

-- doctor_patients
CREATE POLICY "service_role_all_doctor_patients"
  ON doctor_patients FOR ALL
  USING (true);

-- medicine_orders
CREATE POLICY "service_role_all_medicine_orders"
  ON medicine_orders FOR ALL
  USING (true);

-- appointments (may already have policies — add if missing)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'appointments' AND policyname = 'service_role_all_appointments'
  ) THEN
